...
class DagsterCloudClient {
  ...
  async updateLocation(document) {
    ...
    if (locationNames.includes(document.location_name)) {
      const gqlResult = await this.gqlClient.request(UPDATE_LOCATION_MUTATION, {
        document
      });
      result = gqlResult.updateLocationFromDocument;
    } else {
      const gqlResult = await this.gqlClient.request(ADD_LOCATION_MUTATION, {
        document
      });
      result = gqlResult.addLocationFromDocument;
    }
...
module.exports = {DagsterCloudClient};
